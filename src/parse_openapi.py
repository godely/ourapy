...
def _parse_method_details(method_details):
    ...
    # Parse parameters
    if "parameters" in method_details and isinstance(
        method_details["parameters"], list
    ):
...
def _parse_paths(spec):
    ...
    if "paths" not in spec or not isinstance(spec["paths"], dict):
        print(
            "Warning: 'paths' attribute not found or is not a dictionary in the OpenAPI spec."
        )
        return paths_data
    ...
    for path_string, path_item in spec["paths"].items():
        ...
        for method, method_details in path_item.items():
            if method.upper() in [
                ...
                paths_data[path_string][method.upper()] = _parse_method_details(
                    method_details
                )
...
def _parse_schemas(spec):
    ...
    if (
        ...
        and isinstance(spec["components"]["schemas"], dict)
    ):
        for schema_name, schema_details in spec["components"]["schemas"].items():
            components_schemas_data[schema_name] = schema_details
    else:
        print(
            "Warning: 'components.schemas' attribute not found or is not a dictionary in the OpenAPI spec."
        )
