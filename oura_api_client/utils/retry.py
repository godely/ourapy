...
from ..exceptions import OuraRateLimitError, OuraServerError, OuraConnectionError, OuraTimeoutError
...
def should_retry(exception: Exception, attempt: int, max_retries: int) -> bool:
    ...
    if isinstance(exception, OuraRateLimitError):
        if exception.retry_after and exception.retry_after <= 300:  # Max 5 minutes
            return True
        elif not exception.retry_after:  # No retry-after header, use exponential backoff
            return True
